...
const ShareButton = (props) => {
  return (
    <svg
      ...
      fill="none"
    >
      <g clip-path="url(#clip0_1094_933)">
        <path
...
function ShareFavCommentButtons ({ comic, user }) {
  ...
  useEffect(() => {
    ...
    axios
      .post(
        "http://localhost:3000/api/favourite/check",
        { comicId: comic.id },
      ...
      .catch((err) => {
        console.log("Error checking favourite:");
      });
  }, [comic, user]);
  ...
  const toggleFavourite = () => {
    axios
      ...
      .catch((err) => {
        console.log("Error toggling favourite:", err.response);
      });
  };
  return (
